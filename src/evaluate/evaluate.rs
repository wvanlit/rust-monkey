use crate::evaluate::object::{Environment, Object};
...
impl Evaluator {
    ...
    fn eval_expression(&mut self, expression: &mut ast::Expr) -> Object {
        match expression {
            ...
            ast::Expr::If(condition, then, else_option) => {
                self.eval_if_expression(condition, then, else_option)
            }
            ast::Expr::FunctionLiteral(parameters, body) => Object::Function {
        ...
        match data {
            Object::Array(elem) => match index {
                ...
                _ => Object::Error(format!(
                    "Index is not an Integer, instead found: {}",
                    index.type_string()
                )),
            },
            ...
            _ => Object::Error(format!(
                "Expression is not an Array, instead found: {}",
                data.type_string()
            )),
        }
    }
    ...
    fn eval_infix_expression(&self, operator: &TokenType, left: Object, right: Object) -> Object {
        ...
        let type_error = Object::Error(format!(
            "type mismatch: {} {} {}",
            left.type_string(),
            operator,
            right.type_string()
    ...
    fn eval_if_expression(
        &mut self,
        mut condition: &mut Box<ast::Expr>,
        mut then: &mut Box<ast::Statement>,
        else_option: &mut Option<Box<ast::Statement>>,
    ) -> Object {
    ...
    fn eval_call_expression(
        &mut self,
        function: &mut Box<ast::Expr>,
        arguments: &mut Vec<ast::Expr>,
    ) -> Object {
    ...
    fn apply_function(&mut self, function_object: Object, args: Vec<Object>) -> Object {
        match function_object {
            ...
            Object::Builtin(_, paramcount, func) => {
                if paramcount != (args.len() as i32) && paramcount != -1 {
                    return Object::Error(format!(
                        "wrong number of arguments, got {} instead of {}",
                        args.len(),
                        paramcount
...
mod tests {
    ...
    fn test_function_call() {
        let input = vec![
            ...
            ("fn(x){x;}(5);", 5),
            (
                "let add = fn (x) { fn(y){return x+y; }; }; let addFive = add(5); addFive(2)",
                7,
            ),
        ];
    ...
    fn test_string_literal() {
        ...
        match evaluated {
            ...
            _ => assert!(
                false,
                "Expected String Object, got {:?} instead!",
                evaluated
            ),
        }
    }
    ...
    fn test_builtin_functions() {
        let input = vec![
            ...
            (
                r#"len("hello", "world")"#,
                error("wrong number of arguments, got 2 instead of 1"),
            ),
        ];
